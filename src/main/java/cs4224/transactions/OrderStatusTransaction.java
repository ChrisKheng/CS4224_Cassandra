...
public class OrderStatusTransaction extends BaseTransaction {
    ...
    public void execute(String[] dataLines,  String[] parameters) {
        ...
        if (customer != null) {
            ...
            //2
            // TODO: check if customer_order is a better table for this
            String queryGetCustomerLastOrder = String.format(
                    "SELECT O_ID, O_ENTRY_D, O_CARRIER_ID FROM orders " +
                            "WHERE O_W_ID = %d and O_D_ID = %d and O_C_ID = %d and O_CARRIER_ID > -1 " +
            ...
            if (lastOrder != null ) {
                ...
                System.out.printf("Last order's ID: %d, entry time: %s, carrier's ID: %d \n", orderNumber, entryTime, carrierId);
                ...
                for (Row item : items) {
                    ...
                    String deliveryDate = item.getInstant("OL_DELIVERY_D").toString();
